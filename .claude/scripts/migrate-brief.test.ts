...
test('migrates v1 brief text references to v2 IDs', () => {
  const result = migrateBriefRaw(
    [
      ...
      '',
      '## Conflicts',
      '- Runtime version needs confirmation',
      '',
      '## Unknowns',
  ...
  assert.match(result.migrated, /- \[CF1\] Runtime version needs confirmation/);
  assert.match(result.migrated, /- \[UK1\] User approval before committing/);
  assert.match(
    result.migrated,
...
test('migration reports ambiguous duplicate Plan text', () => {
  ...
  assert(
    result.errors.some(error => error.includes('Plan item text is duplicated')),
  );
});
